...
export default {
      ...
      this.shopInfo.recommend.forEach(recommend => {
        recommend.items.forEach(item => {
          if (item.count>0) {
            ...
            this.addTotalCount(item.count);
            this.addTotalPrice(item.activity.fixed_price * item.count)
            this.addSelectFoods(item)
          }
        });
      });
      this.shopInfo.menu.forEach(menu => {
        menu.foods.forEach(food => {
          if (food.count) {
            ...
            this.addTotalCount(food.count)
             this.addTotalPrice(food.activity.fixed_price * food.count)
             this.addSelectFoods(food)
          }
        });
      });
      return empty;
  ...
  methods:{
      ...
      addSelectFoods(food1){
          this.selectFoods.push(food1)
      },
       clearFoods(){
          this.shopInfo.recommend.forEach(recommend=>{
              recommend.items.forEach(item=>{
